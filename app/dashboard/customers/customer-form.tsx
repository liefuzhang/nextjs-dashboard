...
import { useCreateCustomer, useUpdateCustomer } from '@/app/lib/queries'
...
export default function CustomerForm({ 
  ...
  // Form values state
  const [formValues, setFormValues] = useState<CustomerField>({
    id: customer?.id || "",
    name: customer?.name || "",
    ...
    if (isEditing) {
      updateMutation.mutate({ id: customer!.id, formData }, {
        onSuccess: (result) => {
          ...
          } else if (result && 'message' in result && result.message.includes('Error')) {
  ...
  const handleInputChange = (field: keyof CustomerField, value: string) => {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div id="company-error" aria-live="polite" aria-atomic="true">
            {errors.company &&
              errors.company.map((error: string) => (
                <p className="mt-2 text-sm text-red-500" key={error}>
                  {error}
                </p>
              ))}
